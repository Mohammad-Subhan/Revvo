...
const ownerMenuLinks = [
    ...
    { name: "Add car", path: "/owner/cars/add", icon: assets.addIcon, coloredIcon: assets.addIconColored },
    { name: "Manage Cars", path: "/owner/cars/manage", icon: assets.carIcon, coloredIcon: assets.carIconColored },
    { name: "Manage Bookings", path: "/owner/bookings/manage", icon: assets.listIcon, coloredIcon: assets.listIconColored },
]
...
const SideBar = () => {
    ...
    return (
        <div className="relative min-h-screen md:flex flex-col items-center pt-8 max-w-13 md:max-w-60 w-full border-r border-border-color text-sm">
            ...
            {image && (
                <button className="absolute top-0 right-0 flex p-2 gap-1 bg-primary/10 text-primary cursor-pointer">Save <img src={assets.check_icon} width={13} onClick={updateImage} alt="" /></button>
            )}
